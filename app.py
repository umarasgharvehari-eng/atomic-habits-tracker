...
# PAGE: MANAGE
    ...
    <div class="main-header">
        ...
        <p>Edit, pause, stop for today, reorder, or export your systems</p>
    </div>
    """, unsafe_allow_html=True)
...
# PAGE: PROFILE
    ...
    <div class="main-header">
        <h1>👤 My Profile</h1>
        <p>Your identity, motivation, and personal habit system</p>
    </div>
    """, unsafe_allow_html=True)

    user_settings = data.get("user_settings", {})
    profile_name = user_settings.get("name", "").strip() or "Habit Builder"
    ...
    st.markdown(f'<div class="card"><h3 style="margin-top:0;">{profile_name}</h3><div class="small-muted">Level {level} • Weekly goal {weekly_goal}% • Total check-ins {total_checkins}</div></div>', unsafe_allow_html=True)
    ...
    paused_habits = [h for h in data["habits"] if not h.get("active", True)]
    today_summary = get_today_progress_summary(data)
    badges = get_achievements(data)
    ...
    with c3:
        st.metric("Paused habits", len(paused_habits))
    with c4:
        st.metric("Badges", len(badges))

    c5, c6, c7 = st.columns(3)
    with c5:
        st.metric("Today handled", f"{today_summary['handled']}/{today_summary['total']}")
    with c6:
        best_streak_overall = max([get_best_streak(h["id"], data["check_ins"], habit_lookup) for h in data["habits"]] or [0])
        st.metric("Best streak", best_streak_overall)
    with c7:
        st.metric("This week avg", f"{weekly_avg}%")
    ...
    with left:
        ...
        if active_habits:
            ...
            st.info("Add an active habit to build identity statements here.")

    with right:
        st.subheader("Current habit list")
        if data["habits"]:
            for habit in data["habits"]:
                status = "Active" if habit.get("active", True) else "Paused"
                st.write(f"- **{habit['name']}** • {habit.get('category','Personal')} • {status}")
        else:
            st.info("No habits created yet.")

    st.markdown("---")
    st.subheader("Recent self-check data")
    if data.get("daily_logs"):
        recent_dates = sorted(data["daily_logs"].keys(), reverse=True)[:5]
        for date_str in recent_dates:
            log = data["daily_logs"].get(date_str, {})
            reflection = data.get("reflections", {}).get(date_str, "")
            with st.expander(f"{date_str} • {log.get('mood', 'No mood')} • {log.get('energy', 'No energy')}"):
                if log.get("win"):
                    st.write(f"**Win:** {log['win']}")
                if log.get("obstacle"):
                    st.write(f"**Obstacle:** {log['obstacle']}")
        ...
        st.info("Your daily check-ins will appear here.")
